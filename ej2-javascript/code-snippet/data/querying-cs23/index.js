...
var footerFn = ej.base.compile('<tr><td></td><td></td><td>Maximum: ${max}</td></tr>');
...
new ej.data.DataManager({ url: SERVICE_URI, adaptor: new ej.data.ODataV4Adaptor })
    .executeQuery(new ej.data.Query().take(5).requiresCount().aggregate('max', 'Freight'))
    .then((e) => {
        (e.result).forEach((data) => {
            table.appendChild(compiledFunction(data)[0]);
        });
        table.appendChild(footerFn({ max: e.aggregates['Freight - max'] })[0]);
    });