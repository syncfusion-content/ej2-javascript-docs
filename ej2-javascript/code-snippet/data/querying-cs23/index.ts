...
let footerTemplate: string = '<tr><td></td><td></td><td>Maximum: ${max}</td></tr>';
...
new DataManager({
    ...
    new Query().take(5).requiresCount().aggregate('max', 'Freight')
).then((e: ReturnOption) => {
    (e.result as Record<string, any>[]).forEach((data: Record<string, any>) => {
        table.appendChild(compiledFunction(data)[0]);
    });
    let max: number = e.aggregates ? e.aggregates['Freight - max'] : 0;
    table.appendChild(footerFn({ max })[0]);
});