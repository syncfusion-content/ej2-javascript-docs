import { Grid, Toolbar, Filter, Sort, DomVirtualization } from '@syncfusion/ej2-grids';
import { Slider } from '@syncfusion/ej2-inputs';
import { ProgressBar } from '@syncfusion/ej2-progressbar';
import { productData } from './datasource.ts';

Grid.Inject(Toolbar, Filter, Sort, DomVirtualization);

// Template function for SKU ID with icon
(<{ skuTemplate?: Function }>window).skuTemplate = (data: any): string => {
    if (!data || !data.SKUID) {
        return '';
    }
...
(<{ productTemplate?: Function }>window).productTemplate = (data: any): string => {
...
(<{ categoryTemplate?: Function }>window).categoryTemplate = (data: any): string => {
    ...
    const className = `category-${category.toLowerCase()}`;
    return `<div class="category-badge ${className}">${category}</div>`;
};

// Template function for available stock progress bars
(<{ stockTemplate?: Function }>window).stockTemplate = (data: any): string => {
    const stock = data.AvailableStock;
    const progressId = 'progress-' + data.SKUID.replace(/[^a-zA-Z0-9]/g, '');
...
let grid: Grid = new Grid({
    ...
    queryCellInfo: (args: any) => {
        if (args.column.field === 'AvailableStock' && args.data) {
            const stock = args.data.AvailableStock;
            const maxStock = 1500;
            ...
            const progressId = 'progress-' + args.data.SKUID.replace(/[^a-zA-Z0-9]/g, '');
            const element = args.cell.querySelector('#' + progressId);
            ...
            if (element && !element.classList.contains('e-progressbar')) {
                ...
                progressBar.appendTo(element as HTMLElement);
            }
        }
    },
    columns: [
...
const rowBufferSlider: Slider = new Slider({
    min: 1,
    max: 20,
    value: 5,
    type: 'MinRange',
