---
layout: post
title: How to customize in ##Platform_Name## Color Picker | Syncfusion
description: Customize the Syncfusion ##Platform_Name## Color Picker palette by setting presetColors and adding custom tile classes via the beforeTileRender event.
...
By default, the input area will be rendered in ColorPicker. To hide the input area from it, add `e-hide-value` class to ColorPicker using the [`cssClass`](../../api/color-picker/index-default#cssclass) property.
...
In the following sample, the `picker` icon is added to primary button and using [`change`](../../api/color-picker/index-default#change) event the selected color will be updated in bottom portion of the icon.
