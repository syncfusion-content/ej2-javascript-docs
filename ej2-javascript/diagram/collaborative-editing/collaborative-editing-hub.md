...
This guide explains how to configure ASP.NET Core SignalR with Redis for real-time collaborative editing in a ##Platform_Name## application. It covers creating the app, installing packages, configuring SignalR, implementing the hub, handling conflicts, and managing Redis cleanup.
...
Refer to the `RedisService` file in the `Services` folder of the [##Platform_Name## Diagram Collaborative Editing GitHub sample](https://github.com/SyncfusionExamples/ej2-web-diagram-examples/tree/master/CollaborativeEditing), and add the `RedisService` to the `Program.cs` file.
