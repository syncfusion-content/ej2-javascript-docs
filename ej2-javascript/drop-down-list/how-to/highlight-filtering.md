---
layout: post
title: How to highlight match in ##Platform_Name## Dropdown List | Syncfusion
description: Highlight matching characters in Syncfusion ##Platform_Name## Dropdown List filtering using the highlightSearch method on the filtering event.
platform: ej2-javascript
control: Highlight filtering 
publishingplatform: ##Platform_Name##
documentation: ug
domainurl: ##DomainURL##
---

# How to highlight matched character in ##Platform_Name## Dropdown List

By using the `highlightSearch` method, you can highlight the matched characters in the DropDownList filtering.

The following example demonstrates how to highlight the matched characters while filtering.

{% if page.publishingplatform == "typescript" %}

 {% tabs %}
{% highlight ts tabtitle="index.ts" %}
{% include code-snippet/dropdownlist/basic-cs10/index.ts %}
{% endhighlight %}
{% highlight html tabtitle="index.html" %}
{% include code-snippet/dropdownlist/basic-cs10/index.html %}
{% endhighlight %}
{% endtabs %}
        
{% previewsample "page.domainurl/code-snippet/dropdownlist/basic-cs10" %}

{% elsif page.publishingplatform == "javascript" %}

{% tabs %}
{% highlight js tabtitle="index.js" %}
{% include code-snippet/dropdownlist/basic-cs10/index.js %}
{% endhighlight %}
{% highlight html tabtitle="index.html" %}
{% include code-snippet/dropdownlist/basic-cs10/index.html %}
{% endhighlight %}
{% endtabs %}

{% previewsample "page.domainurl/code-snippet/dropdownlist/basic-cs10" %}
{% endif %}