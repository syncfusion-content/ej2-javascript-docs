---
layout: post
title: Configure Work in ##Platform_Name## Gantt Chart Control | Syncfusion
description: Learn how to configure work and task types in the Syncfusion ##Platform_Name## Gantt Chart control for accurate task scheduling and resource allocation.
...
# Configure Work in ##Platform_Name## Gantt Chart Control

The work feature in the ##Platform_Name## Gantt Chart control defines the total effort required to complete a task, mapped via the [taskFields.work](../api/gantt/taskFields#work) property to a numeric value (e.g., 40 for 40 hours). The effort is measured in hours, days, or minutes using the [workUnit](../api/gantt#workUnit) property, with **Hour** as the default. For example, a task with 40 hours of work and one resource at 100% allocation spans 5 8-hour workdays. Mapping `taskFields.work` sets the task type to **FixedWork** by default, ensuring work hours remain constant during edits. This feature, requiring `Edit` service for modifications, integrates with resources, dependencies, and critical path calculations, ensuring precise project scheduling for resource-driven projects.

## Configure work

Use the [taskFields.work](../api/gantt/taskFields#work) property to map a numeric work value (e.g., hours) from the data source. Set the measurement unit with [workUnit](../api/gantt/index-default#workunit) to **Hour**, **Day**, or **Minute** to define how work is calculated. For example, `workUnit: "Day"` assumes 8-hour workdays. Inject `Edit` service to enable editing work via dialogs or taskbar drags. Work values determine taskbar lengths and influence dependency scheduling, aligning project timelines with resource availability.
...
The [taskType](../api/gantt/index-default#tasktype) property controls how `work`, `duration`, and `resourceUnit` fields interact during edits, with `FixedUnit` as the default unless `taskFields.work` is mapped (sets **FixedWork**). Inject `EditService` to enable editing. The available task types are:
...
- Work calculations rely on [resourceInfo](../api/gantt/taskFields#resourceinfo) for accurate resource unit allocation.
