...
description: Learn how to perform custom data binding in the ##Platform_Name## Data Grid with remote data sources and server-side data operations.
...
The custom binding feature in the Data Grid enables managing a custom API for handling data processing externally and then binding the resulting data to the grid. When using custom binding, the grid expects the result to be an object with `result` (array of records to display) and `count` (total number of records) properties.

The Data Grid component offers a range of powerful features such as **paging**, **grouping**, **sorting**, and **filtering**. These actions trigger the [dataStateChange](../../api/grid#datastatechange) event. CRUD actions such as **Create**, **Read**, **Update**, and **Delete** trigger the (../../api/grid#datasourcechanged) event. These events provide an opportunity to manage and manipulate data according to interactions.

**Using the dataStateChange event**

The `dataStateChange` event is triggered whenever an action modifies the state of the grid's data, such as changing pages, applying sorting, or grouping. This event provides detailed information about the action performed and the current state of the grid, including parameters like page number, sorting details, and filtering criteria.
...
When a filtering operation is performed in the grid, the `dataStateChange` event is triggered, providing access to the following arguments within the event.

![FilterBar](../images/databining/remote-filter.png)

The grid data state for the filter action can be updated as follows:
...
When filtering multiple values, the `dataStateChange` event returns the predicates as arguments. Predicate execution can be created based on those values.

## Handling searching operation

When a search operation is performed in the grid, the `dataStateChange` event is triggered, allowing access to the following arguments within the event.

![Searching](../images/databining/remote-serach.png)

The grid data state for the search action can be updated as follows:
...
When a sorting operation is performed in the grid, the `dataStateChange` event is triggered, and within this event the following arguments are available.

![Sorting](../images/databining/remote-sort.png)

When performing multi-column sorting, the `dataStateChange` event returns the following arguments.

![Multi Sorting](../images/databining/remote-multiple-sort.png)

The grid data state for the sort action can be updated as follows:
...
When a paging operation is performed in the grid, the `dataStateChange` event is triggered, and within this event the following arguments are available.

![Paging](../images/databining/remote-paging.png)

The grid data state for the page action can be updated as follows:
...
When a grouping operation is performed in the grid, the `dataStateChange` event is triggered, providing access to the following arguments within the event.

![Grouping](../images/databining/remote-group.png)

The grid data state for the group action can be updated as follows:
...
In ##Platform_Name##, lazy loading refers to the technique of loading data dynamically when needed, instead of loading everything upfront. Lazy load grouping allows grouped data to be loaded and displayed efficiently by fetching only the required data on demand.  

To enable this feature, set the [groupSettings.enableLazyLoading](../../api/grid/groupSettings#enableLazyLoading) property to **true**. Also, manage the state based on the initial grid action as follows.
{% if page.publishingplatform == "typescript" %}
```typescript
...
Based on the initial state, the arguments are as shown below.

![Lazy load group](../images/databining/remote-lazyload.png)

The grid state can be updated as follows:
{% if page.publishingplatform == "typescript" %}
...
The Data Grid component provides powerful options for dynamically inserting, deleting, and updating records, enabling data to be modified directly within the grid. This feature is useful for performing CRUD (Create, Read, Update, Delete) operations seamlessly.

Integrating CRUD Operations

To implement CRUD operations using grid, follow these steps:

1. **Configure grid settings:** Set up the required grid settings, such as editing, adding, and deleting records. Define the toolbar options to support the workflow.

2. **Handle data state changes:** Use the [dataStateChange](../../api/grid#datastatechange) event to respond to changes in the grid’s data state. This event is triggered when grid actions such as paging or sorting occur.

3. **Execute CRUD operations:** Within the event handler for [dataSourceChanged](../../api/grid#datasourcechanged), implement logic to handle CRUD actions based on the action or requestType property of the event arguments.

4. **Call endEdit method:** After performing CRUD operations, call the endEdit method to complete the operation and update the grid accordingly.
...
By default, the Data Grid component exports only the records available on the current page. However, the Data Grid component allows exporting all records, including those from multiple pages, by configuring the [pdfExportProperties](../../api/grid/pdfExportProperties) and [excelExportProperties](../../api/grid/excelExportProperties).
...
The following code example shows exporting all records in client side:
...
The Data Grid component enables inclusion of custom parameters in data requests. This feature is particularly useful when additional information must be sent to the server for processing.

By using the [query](../../api/grid#query) property of the grid along with the `addParams` method of the `Query` class, custom parameters can be incorporated into data requests for every grid action.

To enable custom parameters in data requests for the grid, follow these steps:

**1. Bind the Query Object to the Grid**:  Assign the initialized query object to the `query` property of the Data Grid component.
...
The following example demonstrates sending additional parameters to the server.
...
To configure a server with ##Platform_Name## Grid, follow the below steps:

{% if page.publishingplatform == "typescript" %}


**Step 1:** Open the terminal and create a new Typescript project using the following command:
...
**Step 6:** Update the scripts section in the **package.json** to include commands for starting the server and client concurrently:
...
**Step 9:** Create an **orderService.ts** file inside the **src** folder (e.g., `src/orderService.ts`) to handle CRUD operations and grid actions using `DataManager`:
...
**Step 10:**  In the **app.ts** file, import and use the Data Grid component. Use the **orderService** to fetch and manipulate data.
...
**Step 11:** Finally, start both the server and client concurrently using the start script.
...
> Replace https://localhost:xxxx/orders with the actual URL of the API endpoint that provides the data in a consumable format (e.g., JSON).
...
**Step 2:**  Create an **index.html** file under the root folder and add the necessary HTML structure along with CSS and JavaScript links to include grid dependencies.
...
**Step 9:** Create an **orderService.js** file to handle CRUD operations and grid actions using `DataManager`:
...
**Step 10:**  In the **index.js** file, import and use the grid component. Use the **orderService** to fetch and manipulate data.
...
By default, Data Grid automatically binds a remote data source using the [DataManager](../../data/getting-started). However, in some scenarios, it may be necessary to fetch data dynamically from the server using a query triggered by an external button. This approach provides greater control over when and the manner in which data loads into the grid.

To achieve this, use the [executeQuery](../../api/data/dataManager#executequery) method of `DataManager` with a query object. This method enables running a custom query and retrieving results dynamically.
