...
By default, ##Platform_Name## Grid automatically converts date values to the local time zone of the client system. However, in some scenarios, you may need to display the original date as received from the server without any timezone conversion.
...
The following example demonstrates how to prevent local time zone conversion for date columns in ##Platform_Name## Grid by using the `DataUtil.serverTimezoneOffset` property:
