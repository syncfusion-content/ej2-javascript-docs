---
layout: post
title: Configuration in ##Platform_Name## In-place Editor | Syncfusion
description: Configure ##Platform_Name## In-place Editor rendering modes, popup title and position, edit triggers (Click, DblClick, EditIconClick), and focus-out actions.
...
# Configuration in ##Platform_Name## In-place Editor
...
The event action of the editor that enable in the edit mode based on the [editableOn](../api/inplace-editor/index-default#editableon) property, by default `Click` is assigned, the following options are also supported.
...
Action to be performed when the user clicks outside the container, that means focusing out of editable content and it can be handled by the [actionOnBlur](../api/inplace-editor/index-default#actiononblur) property, by default `Submit` assigned. It also has the following options.
