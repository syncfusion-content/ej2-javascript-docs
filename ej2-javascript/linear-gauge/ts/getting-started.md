...
The Linear Gauge component is available in the `@syncfusion/ej2-lineargauge` package. The following dependencies are used by the package:
...
Locate the `src/index.html` file. Preserve the existing content generated by the seed project and add a `<div>` element with the ID `element` inside the `<body>` element.
...
### Step 6: Create the Linear Gauge Component
...
After completing the quick setup, the browser displays a Linear Gauge with its default axis.
...
* **`Cannot find module '@syncfusion/ej2-lineargauge'`.** Re-run `npm install` from the project root. For a custom project, install the package with `npm install @syncfusion/ej2-lineargauge --save`.
...
* **The Linear Gauge is not displayed.** Ensure that the HTML container ID is `element` and that `appendTo('#element')` uses the same ID.
