---
layout: post
title: Legend in ##Platform_Name## Maps | Syncfusion
description: Learn here all about Legend in Syncfusion ##Platform_Name## Maps of Syncfusion Essential JS 2 and more.
...
The legend supports two types of modes.
1. **Default** mode
2. **Interactive** mode
...
The following code snippet demonstrates the equal color mapping legends for the shapes. Bind the given data source to the [`dataSource`](../api/maps/layerSettingsModel/datasource) property of [`layerSettings`](../api/maps/layerSettingsModel). Set the value of [`shapePropertyPath`](../api/maps/layerSettingsModel/shapepropertypath) to **name** and [`shapeDataPath`](../api/maps/layerSettingsModel/shapedatapath) to **Country**. To enable equal color mapping, set the [`colorMapping`](../api/maps/colorMappingSettingsModel) as an array in [`shapeSettings`](../api/maps/shapeSettingsModel). Finally, set the [`visible`](../api/maps/legendSettingsModel/visible) property of [`legendSettings`](../api/maps/legendSettingsModel) to **true**. The [`label`](../api/maps/colorMappingSettingsModel/label) property in [`colorMapping`](../api/maps/colorMappingSettingsModel) is used to set the text name for the legend in Maps.
...
The toggle option has been provided for the legend. When the legend is toggled, the given color will be changed to the corresponding Maps shape item. To enable the toggle options in the legend, set the [`enable`](../api/maps/toggleLegendSettingsModel/enable) property of the [`toggleLegendSettings`](../api/maps/toggleLegendSettingsModel) to **true**.
...
To enable the legend for bubbles, set the [`visible`](../api/maps/legendSettingsModel/visible) property of [`legendSettings`](../api/maps/legendSettingsModel) to **true** and the [`type`](../api/maps/legendSettingsModel/type) property of [`legendSettings`](../api/maps/legendSettingsModel) to **Bubbles**.
...
To enable the legend for markers, set the [`visible`](../api/maps/legendSettingsModel/visible) property of [`legendSettings`](../api/maps/legendSettingsModel) to **true** and the [`type`](../api/maps/legendSettingsModel/type) property of [`legendSettings`](../api/maps/legendSettingsModel) to **Markers**. The [`legendText`](../api/maps/markerSettingsModel/legendtext) property in the [`markerSettings`](../api/maps/markerSettingsModel) can be used to show the legend text based on values provided in the data source.
