...
2. ZoomIn - Zooms in the map.
3. ZoomOut - Zooms out the map.
4. Pan - Switches to panning if rectangular zoom is activated.
5. Reset - Restores the map to the default view.
...
* [`border`](../api/maps/selectionSettingsModel/border) - To customize the color, width, and opacity of the border of the element that is selected in Maps.
...
* [`enableMultiSelect`](../api/maps/selectionSettingsModel/enablemultiselect) - To enable or disable the selection for multiple shapes, markers, or bubbles in the Maps.

By tapping on a specific legend, the shapes that are bound to the selected legend are also selected and vice versa.
...
The [`shapeSelection`](../api/maps/shapeselection) method can be used to select each shape in the Maps. The input parameters for this method are layer index, property name, property value (such as country name), and a boolean value (true/false) indicating the selection state.
...
Shapes can be initially selected when the Maps loads using the [`initialShapeSelection`](../api/maps/initialShapeSelectionSettingsModel) property. The values are mapped to the [`shapePath`](../api/maps/initialShapeSelectionSettingsModel/shapepath) and [`shapeValue`](../api/maps/initialShapeSelectionSettingsModel/shapevalue) properties.
...
Using the [`initialMarkerSelection`](../api/maps/initialMarkerSelectionSettingsModel) property, markers can be selected initially when the Maps loads. Markers are rendered based on the [`latitude`](../api/maps/initialMarkerSelectionSettingsModel/latitude) and [`longitude`](../api/maps/initialMarkerSelectionSettingsModel/longitude) values.
...
Each shape in the Maps can be highlighted during mouse hover or touch interactions. This feature applies to Maps elements such as shapes, bubbles, markers, and legends. Highlight is enabled by setting the [`enable`](../api/maps/highlightSettingsModel/enable) property of [`highlightSettings`](../api/maps/highlightSettingsModel) to **true**.
...
* [`border`](../api/maps/highlightSettingsModel/border) - To customize the color, width, and opacity of the border of the element that is highlighted in Maps.
...
When hovering on a specific legend, the shapes that are bound to the selected legend are also highlighted and vice versa.
...
On mouse over or touch end events, the tooltip is used to display additional information about the layer, bubble, or marker. To enable tooltips in Maps, the **Tooltip** module must be injected into Maps using the **Maps.Inject(Tooltip)** method. Tooltips can be enabled separately for layers, bubbles, or markers by setting the [`visible`](../api/maps/tooltipSettingsModel/visible) property of [`tooltipSettings`](../api/maps/tooltipSettingsModel) to **true**. The [`valuePath`](../api/maps/tooltipSettingsModel/valuepath) property specifies the field name from the data source whose value is displayed as the tooltip text. The [`tooltipDisplayMode`](../api/maps/mapsModel/tooltipdisplaymode) property is used to change the display mode of the tooltip in Maps. The following display modes are available in the Maps component. By default, [`tooltipDisplayMode`](../api/maps/mapsModel/tooltipdisplaymode) is set to **MouseMove**.
