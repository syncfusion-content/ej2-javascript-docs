...
Users can display the pivot chart component individually with pivot values and modify the report dynamically using the field list and grouping bar. The [`displayOption`](https://ej2.syncfusion.com/documentation/api/pivotview/displayOptionModel) property in the Pivot Table allows users to control the visibility of both the grid and chart components. This property includes the following options:

* [`view`](https://ej2.syncfusion.com/documentation/api/pivotview/displayOptionModel#view): Determines whether the Pivot Table component displays only the grid, only the chart, or both components.
* [`primary`](https://ej2.syncfusion.com/documentation/api/pivotview/displayOptionModel#primary): Specifies which component (grid or chart) appears as the primary view during initial loading. This option applies only when the [`view`](https://ej2.syncfusion.com/documentation/api/pivotview/displayOptionModel#view) property is set to **Both**.
...
The Pivot Table component supports both local and remote data binding options to populate data in the pivot chart. Users can bind data to the component using the [`dataSource`](https://ej2.syncfusion.com/documentation/api/pivotview/dataSourceSettings#datasource) property, which accepts either an instance of [`DataManager`](https://ej2.syncfusion.com/documentation/api/data/dataManager) for remote data sources or a Typescript object array collection for local data. For further details, [refer here](./data-binding).
...
By default, the **Line** chart type is displayed in the Pivot Chart. Users can change the chart type at any time using the [`type`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotSeries#type) property under [`chartSeries`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotSeries).
...
You can use any of these chart types to visualize your aggregated data clearly. To select a specific accumulation chart, set the [`type`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotSeriesModel#type) property in the [`chartSeries`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotSeriesModel) option.
...
In accumulation charts, only the values from a single column in the pivot chart are displayed. By default, the first column is used. If you want to show values from a different column, you can specify the column headers with the [`columnHeader`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettingsModel#columnheader) property in [`chartSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/index-default#chartsettings).

If the column has more than one header, enter all the headers separated by a delimiter, such as **Germany-Road Bikes**. You can set your preferred delimiter using the [`columnDelimiter`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettingsModel#columndelimiter) property in [`chartSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/index-default#chartsettings). This allows you to display the correct values in your accumulation chart according to how your columns are grouped in the Pivot Table.
...
By default, the data labels in accumulation charts display the header name. You can control their visibility using the [`visible`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotChartDataLabelModel#visible) property in the [`dataLabel`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotSeriesModel#datalabel) settings.

To improve label arrangement and prevent overlapping, the **Smart Labels** option arranges labels efficiently. You can disable this option by setting the [`enableSmartLabels`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettingsModel#enablesmartlabels) property to **false** in the [`chartSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/index-default#chartsettings).

The [`position`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotChartDataLabelModel#position) property in [`dataLabel`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotSeriesModel#datalabel) allows you to specify where the data label appears. The available options are:
...
The **Connector Line** appears when data labels are positioned outside the chart. You can style this connector line using the [`connectorStyle`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotChartDataLabelModel#connectorstyle) property in [`dataLabel`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotSeriesModel#datalabel) to modify its color, length, width, and other properties. In the example below, the connector line appearance is changed to show a different style.
...
You can draw pie and doughnut charts within a specific range by using the [`startAngle`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotSeriesModel#startangle) and [`endAngle`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotSeriesModel#endangle) properties in the [`chartSeries`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotSeriesModel) configuration. The default value for the [`startAngle`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotSeriesModel#startangle) property is **0**, and the [`endAngle`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotSeriesModel#endangle) property is **360**. By modifying these properties, you can create semi-pie and semi-doughnut charts.
...
You can convert a pie chart to a doughnut chart and vice-versa using the [`innerRadius`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotSeriesModel#innerradius) property in the [`chartSeries`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotSeriesModel) configuration. When this property is set to a value greater than **0** percent, the chart appears as a doughnut instead of a pie chart.
...
The field list makes it easy to add, remove, or rearrange fields in the pivot chart, so you can display exactly the data you need. To show the field list in the Pivot Chart, set the [`showFieldList`](https://ej2.syncfusion.com/documentation/api/pivotview/index-default#showfieldlist) property of the Pivot Table to **true**. When you make changes with the field list, the pivot chart updates right away to show the new results. To learn more about the field list and how it works, see the [field list](./field-list) topic in the documentation.
...
You can display the grouping bar in the Pivot Chart by setting the [`showGroupingBar`](https://ej2.syncfusion.com/documentation/api/pivotview/index-default#showgroupingbar) property to **true**. When enabled, the grouping bar in the pivot chart shows a drop-down list on the value axis. This drop-down lets users select from the value fields defined in [`dataSourceSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/dataSourceSettings). Users can switch between these fields to update the chart based on the selected value field. This method of selection is the default behavior in the Pivot Chart component. For more details about how the grouping bar works, refer to the [grouping bar](./grouping-bar) documentation.
...
By default, the pivot chart uses the first value field (measure) from your report as the value axis. If you want to display data using a different value field, you can do this easily. Use the [`value`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettings#value) property inside [`chartSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettings). This option lets you show a specific value field in the pivot chart, allowing you to focus on the data you need.
...
When binding more value fields, the result displays multiple pivot charts, with each chart shrinking within the parent container height. To prevent this behavior, set the [`enableScrollOnMultiAxis`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettings#enablescrollonmultiaxis) property to **true** in the [`chartSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettings). This ensures each pivot chart maintains a minimum height of "160px" to "180px" and displays a vertical scroll bar for better visibility.
...
Alternatively, you can display multiple values in a single chart. In this approach, the series from multiple values are grouped and displayed together. Based on the values, multiple Y-axis scales are created with different ranges. This can be achieved by setting [`enableMultipleAxis`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettings#enablemultipleaxis) to **true** and [`multipleAxisMode`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettings#multipleaxismode) to `Single` in the [`chartSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettings).
...
You can customize the series in the pivot chart by using the [`chartSeries`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotSeries) property inside [`chartSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettings). Any changes you make to the [`chartSeries`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotSeries) property will apply to all series in the chart.
...
If you want to change each series separately, use the [`chartSeriesCreated`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotViewModel#chartseriescreated) event. This event happens after the pivot chart series are created, making it possible to work with each series one at a time.
...
Users can customize the x-axis and y-axis of the pivot chart using the [`primaryXAxis`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettings#primaryxaxis) and [`primaryYAxis`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettings#primaryyaxis) options in the [`chartSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettings) property of the Pivot Table.
...
Users can also modify multi-level labels on the primary x-axis by using the [`multiLevelLabelRender`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettingsModel#multilevellabelrender) event in [`chartSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettings). This event is triggered whenever a multi-level label is rendered on the chart. The event provides the following options:
...
Users can easily change the legend in the pivot chart by using the [`legendSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotChartSettingsLegendSettings) option inside the [`chartSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettings) property. By default, the legend is shown. If you want to hide it, set the [`visible`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotChartSettingsLegendSettings#visible) property in [`legendSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotChartSettingsLegendSettings) to **false**.
...
The default shape for the legend is **SeriesType**, but you can change it by setting the [`legendShape`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotSeries#legendshape) property in [`chartSeries`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotSeries#legendshape).

Users can also choose where the legend appears in the pivot chart by setting the [`position`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotChartSettingsLegendSettings#position) property in [`legendSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotChartSettingsLegendSettings). The available positions include:
...
You can show and customize markers and crosshair's on the pivot chart. To do this, use the [`marker`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotChartSeriesMarkerSettings) and [`crosshair`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettings#crosshair) options within the [`chartSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettings) property.

If you want to display a tooltip when hovering over an axis crosshair, use the [`crosshairTooltip`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotAxisModel#crosshairtooltip) option.
...
Users can customize the zooming and panning options in the pivot chart by using the [`zoomSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotZoomSettings) property within [`chartSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettings). The pivot chart supports four zooming options:

- [`enablePinchZooming`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotZoomSettingsModel#enablepinchzooming) – Zooms by pinching on touch devices.
- [`enableSelectionZooming`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotZoomSettingsModel#enableselectionzooming) – Zooms into a selected region on the chart.
- [`enableDeferredZooming`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotZoomSettingsModel#enabledeferredzooming) – Applies zooming only after the selection is complete, which helps improve performance for large data.
- [`enableMouseWheelZooming`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotZoomSettingsModel#enablemousewheelzooming) – Zooms using the mouse wheel.

The direction of zooming can be controlled using the [`mode`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotZoomSettings#mode) property in [`zoomSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotZoomSettings). The available options are:

- `x`: zooms horizontally,
- `y`: zooms vertically,
- `x,y`: zooms both horizontally and vertically.

When the pivot chart is zoomed, a toolbar appears at the top of the chart with tools for Zoom, Zoom In, Zoom Out, Pan, and Reset actions. This toolbar can be customized using the [`toolbarItems`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotZoomSettings#toolbaritems) property in [`zoomSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotZoomSettings).
...
The tooltip in the Pivot Chart is enabled by default, showing detailed information about each data point when users move the pointer over the chart. Users can change how the tooltip looks and functions by using the [`tooltip`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotTooltipSettings) option inside the [`chartSettings`](https://ej2.syncfusion.com/documentation/api/pivotview/chartSettings) property.

If users do not want to show the tooltip, they can disable it by setting the [`enable`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotTooltipSettings#enable) property in [`tooltip`](https://ej2.syncfusion.com/documentation/api/pivotview/pivotTooltipSettings) to **false**.
...
You can export the pivot chart to various file formats by using the [`chartExport`](https://ej2.syncfusion.com/documentation/api/pivotview/index-default#chartexport) method. This method requires the `type` parameter, which specifies the file format. The supported export formats are:
...
Other optional parameters for the [`chartExport`](https://ej2.syncfusion.com/documentation/api/pivotview/index-default#chartexport) method include:
...
You can print the current view of the pivot chart directly from the browser by using the [`printChart`](https://ej2.syncfusion.com/documentation/api/pivotview/index-default#printchart) method. This allows you to create a physical or digital copy of your pivot chart as displayed on the screen.

In the example below, clicking an external button labeled "Print Chart" calls the [`printChart`](https://ej2.syncfusion.com/documentation/api/pivotview/index-default#printchart) method and prints the rendered pivot chart.
