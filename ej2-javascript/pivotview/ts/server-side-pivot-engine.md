...
The server-side engine seamlessly supports Excel export functionality, enabling users to efficiently generate and download pivot table reports in Excel format directly from the server. To enable Excel export in the pivot table, set the [`allowExcelExport`](https://ej2.syncfusion.com/documentation/api/pivotview/index-default#allowexcelexport) class to **true**. Once the API is set, the user needs to call the [`excelExport`](https://ej2.syncfusion.com/documentation/api/pivotview/index-default#excelexport) method to export the pivot table to Excel by clicking an external button.
...
The Excel export provides an option to include header and footer content for the excel document before exporting. In-order to add header and footer, define **header** and **footer** properties in **excelExportProperties** object and pass it as a parameter to the [`excelExport`](https://ej2.syncfusion.com/documentation/api/pivotview/index-default#excelexport) method.
...
To enable native Excel pivot export in the PivotTable, the user must call the [`exportAsPivot`](https://ej2.syncfusion.com/documentation/api/pivotview/index-default#exportAsPivot) method to export the PivotTable to Excel by clicking an external button, specifying the export type (**Excel** or **CSV**) as a parameter.
