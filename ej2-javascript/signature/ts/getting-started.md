...
## Install Syncfusion<sup style="font-size:70%">&reg;</sup> Signature package
...
## Import Syncfusion<sup style="font-size:70%">&reg;</sup> Signature CSS styles
...
## Add Syncfusion<sup style="font-size:70%">&reg;</sup> Signature control to the application

Open the application in Visual Studio Code and add the Syncfusion<sup style="font-size:70%">&reg;</sup> JavaScript UI controls. 

In this article, the Signature control is used as an example. Add the HTML Canvas tag with the `id` attribute as `signature` to the `~/src/index.html` file.
...
To render the Signature control, add the following JavaScript code to the `~/src/app/app.ts` file
