...
This mode can be applied to Tab by setting the [`loadOn`](../api/tab#loadon) property to `Dynamic`. In this mode, only the content of the currently selected tab is loaded and available in the DOM initially. When a different tab is selected, its content replaces the current content. Since this mode ensures the DOM maintains only the content of the active tab, page loading performance is improved. However, the Tab do not retain its state, as each time a tab is selected, it loads its content again.
