...
The [`tabIndex`](../api/tab/tabItem#tabindex) property of a Tab item enables tab key navigation for that specific item. When you assign a positive value to the [`tabIndex`](../api/tab/tabItem#tabindex) property, users can switch focus between tab items using the Tab or Shift+Tab keys. By default, users can switch between tab items only using arrow keys.

If you set the [`tabIndex`](../api/tab/tabItem#tabindex) value to 0 for all tab items, the tab will switch based on the order of elements on the page. This means that users can navigate through the tab items using the Tab key in the same order as they appear on the page.

To use the [`tabIndex`](../api/tab/tabItem#tabindex) property, assign a positive value to each tab item for which you want to enable tab key navigation. Here's an example:
...
With this code, users can switch between tab items using the Tab and Shift+Tab keys in the order specified by the [`tabIndex`](../api/tab/tabItem#tabindex) property values.
