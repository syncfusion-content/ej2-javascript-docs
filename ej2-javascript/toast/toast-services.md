...
| [timeOut](../api/toast/#timeout) | Specifies the Toast display time duration on the page in milliseconds. Once the time expires, the Toast message will be removed. Setting 0 as the time out value displays the Toast on the page until it is closed manually. |
...
The utility function can be called using the [ToastModel](../api/toast/toastModel/) as an argument to show the toast, where all the properties in the `ToastModel`, like any events, position, close icon, action buttons, etc., can be used in the `ToastUtility.show`.
