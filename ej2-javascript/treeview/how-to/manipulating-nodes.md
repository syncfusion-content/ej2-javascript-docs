...
The TreeView control has the [`updateNode`](../../api/treeview#updatenode) method, which allows you to change a specific node's text by providing its target (either the node ID or element) and the new text. The [`updateNode`](../../api/treeview#updatenode) method works programmatically regardless of the [`allowEditing`](../../api/treeview#allowediting) property; the `allowEditing` property is only required to enable user-driven in-place text editing (double-click to edit) in the UI.
